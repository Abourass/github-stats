...
export function generateOverview(data: StatsData): string {
  return `<svg id="gh-dark-mode-only" width="360" height="210" xmlns="http://www.w3.org/2000/svg">
<style>
svg {
...
export function generateLanguages(data: StatsData): string {
  const sortedLanguages = Object.entries(data.languages).sort((a, b) => b[1].size - a[1].size);

  const progressBars = sortedLanguages
    .map(
      ([_, langData]) =>
        `<span style="background-color: ${langData.color || '#000000'};width: ${(langData.prop || 0).toFixed(3)}%;" class="progress-item"></span>`
    )
    .join('');
...
<svg xmlns="http://www.w3.org/2000/svg" class="octicon" style="fill:${langData.color || '#000000'};"
viewBox="0 0 16 16" version="1.1" width="16" height="16"><path
fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8z"></path></svg>
<span class="lang">${lang}</span>
<span class="percent">${(langData.prop || 0).toFixed(2)}%</span>
</li>
`
    )
    .join('');
